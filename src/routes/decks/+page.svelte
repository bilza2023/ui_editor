<script>
    // 1) grab the `data` injected by our server loader
    export let data;
      ...
      <tbody>
        {#each decks as deck}
          <tr>
            ...
            <td class="actions">
              <a href={`/player/?filename=${deck.id}`}><button class="btn-play">Player</button></a>
              <a href={`/clone`}><button class="btn-clone">Clone</button></a>
              <a href={`/timings?filename=${deck.id}`}><button class="btn-edit">Timings</button></a>
              <a href={`/delete?filename=${deck.id}`}><button class="btn-delete">Delete</button></a>
            </td>
          </tr>
        {/each}
      </tbody>
